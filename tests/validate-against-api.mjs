...
function listPublicMushafs() {
  ...
  for (const filename of readdirSync(rawisDir).sort()) {
    if (!filename.endsWith('.json')) {
      continue;
    }

    const meta = load(`rawis/${filename}`);
...
async function validateMushaf(mushaf) {
  ...
  const surahCounts = load(`surah-counts/${mushaf.systemId}.json`);
  const reverse = mushaf.systemId === 'kufi'
    ? null
    : load(`mappings/by-counting-system/${mushaf.systemId}-to-kufi.json`);
