/**
 * Compare the word-level scholarly differences file with the authoritative
 * mapping-derived boundary events.
 *
...
const wordLevel = JSON.parse(readFileSync(join(dataDir, 'differences.json'), 'utf-8'));
const boundaryEvents = JSON.parse(readFileSync(join(dataDir, 'boundary-events.json'), 'utf-8'));
const kufiCounts = JSON.parse(readFileSync(join(dataDir, 'surah-counts', 'kufi.json'), 'utf-8'));
...
const report = {
  _version: version,
  _description: 'Reconciliation report between the scholarly word-level differences file and the authoritative mapping-derived boundary-events file.',
...
writeFileSync(join(dataDir, 'differences-reconciliation.json'), JSON.stringify(report, null, 2) + '\n');
console.log('  Generated: data/differences-reconciliation.json');
