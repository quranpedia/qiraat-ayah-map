...
function get_nav_items() {
  return [
    ...
    { href: '/developer', label: 'المطور', icon: FilesIcon, matches: path => path === '/developer' }
  ]
}
  ...
  <div class="editorial_shell page_shell px-5 py-4 sm:px-7">
    <div class="flex items-center justify-between gap-6">
      <a class="flex items-center gap-3" href="/">
        ...
        <div>
          {#if current_language === 'ar'}
            <div class="arabic_title text-lg text-ink">{brand_title}</div>
            <div class="text-sm font-extrabold tracking-[0.2em] text-ink-soft uppercase">{brand_secondary_title}</div>
          {:else}
            <div class="text-sm font-extrabold tracking-[0.2em] text-ink-soft uppercase">{brand_title}</div>
            <div class="arabic_title text-base text-ink">{brand_secondary_title}</div>
          {/if}
        </div>
      </a>
      ...
      <div class="flex items-center gap-2" role="group" aria-label="تبديل اللغة">
        ...
        <button
          type="button"
          class="nav_link rounded-full px-3 py-2 text-sm"
